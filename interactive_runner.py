...
def main_menu(direction, case_file, network):
    ...
    service = Client(asset_preparer.sol_url, timeout=20)

    while True:
        is_sol = direction == "sol_to_evm"
        target_name = "Solana" if is_sol else "EVM"

        print("\n" + "="*50)
        print(f"📬 {target_name} -> {'EVM' if is_sol else 'Solana'} Bridge Menu")
        print("="*50)
        print("1. 🛠️  Create Wallets (SOL + EVM)")
        print(f"2. 🔍 Check {target_name} Balances")
        print(f"3. 💸 Distribute Funds ({target_name} Native & Tokens)")
        print("4. 🚀 Run Transactions")
        print(f"5. 🧹 Sweep Assets ({target_name})")
        ...
        if choice == '1':
            ...
            if check_wallet_coverage(case_file, direction):
                if is_sol:
                    asset_preparer.check_all_sol_balances(case_file, get_solana_wallet_info()[:3])
                else:
                    asset_preparer.check_all_evm_balances(case_file, get_evm_wallet_info()[:3])
            ...
            if is_sol:
                ...
                def send_distribution_batch(recipients, ata_ixs):
                    ...
                    if sig:
                        ...
                        except Exception as e: print(f"    ⚠️ Confirmation wait error: {e}")
                ...
                for i, wallet in enumerate(batch_wallets):
                    ...
                    if '0x000000000000' not in case['from_token_address'] and len(case['from_token_address']) > 20:
                        try:
                            ...
                            curr_ata_ixs.append(transfer_checked(TransferCheckedParams(TOKEN_PROGRAM_ID, main_ata, token_mint, ata_addr, main_kp.pubkey(), case['amount_raw'], case['from_token_decimals'])))
                ...
                asset_preparer.distribute_evm_funds(case_file, get_evm_wallet_info()[:3])
                print("\n🔍 Verifying updated balances...")
                asset_preparer.check_all_evm_balances(case_file, get_evm_wallet_info()[:3])
...
def run_sol_to_evm(case_file, network, service):
    ...
    try: tps = float(input("👉 Enter TPS (e.g. 1.0): ").strip())
    except: tps = 1.0
    ...
    def process_case(idx, wallet, case):
        ...
        try:
            tx, error = solana_to_evm_msg(batch_kp, case.get('to_chain_id', 2153201998), case.get('to_token_address', '0x174BADB1B8b9248dAe0519C5C8f9fFd9aCb2E779'), case['to_address'], case['amount_raw'], case['from_token_address'], network, case.get('gas_limit', 80000))
...
def run_evm_to_sol(case_file):
    ...
    print(f"🚀 Running {len(cases)} EVM -> Solana transactions using batch wallets...")
