...
class PrepareAssets:
    def __init__(self, network='devnet'):
        ...
        try:
            ...
            self.sol_url = rpc_config.get(network, "https://api.devnet.solana.com")
            # For EVM, we might need a separate RPC. Let's try to get it from a testcase if not in config
            self.evm_url = rpc_config.get(f"{network}_evm", "https://gwan-ssl.wandevs.org:46891")
        except:
    ...
    def _get_sol_balance_http(self, addr_str):
        payload = {
            "jsonrpc": "2.0",
            "id": 1,
            "method": "getBalance",
        ...
        try:
            response = requests.post(self.sol_url, json=payload, timeout=20)
    ...
    def _get_sol_token_balance_http(self, addr_str, mint_str):
    ...
    def check_all_sol_balances(self, case_file, sol_wallet_info):
        ...
        token_needs = {}
        for c in cases:
            if '0x000000000000' not in c['from_token_address'] and len(c['from_token_address']) > 20: # Solana pubkey
                mint = c['from_token_address']
                token_needs[mint] = token_needs.get(mint, 0) + c['amount_raw']

        print("\n" + "="*80)
        print(f"🔍 Solana Balance Diagnostics (via RPC: {self.sol_url})")
        print("="*80)

        sol_addr = main_wallet['address']
        balance, err = self._get_sol_balance_http(sol_addr)
        ...
        for mint, needed_raw in token_needs.items():
            try:
                ...
                t_bal, t_err = self._get_sol_token_balance_http(sol_addr, mint)
                if t_err: token_summaries.append(f"{case['token_name']}: {t_err}")
                else:
        ...
        for i, wallet in enumerate(batch_wallets):
            ...
            balance, err = self._get_sol_balance_http(addr_str)
            needed_sol = case['network_fee_raw'] + (0.005 * 10**9)
            sol_status = "❌ ERR" if err else ("✅ OK" if balance >= needed_sol else "⚠️ LOW")

            token_status = "N/A"
            if '0x000000000000' not in case['from_token_address'] and len(case['from_token_address']) > 20:
                t_bal, t_err = self._get_sol_token_balance_http(addr_str, case['from_token_address'])
    ...
    def sweep_sol_assets(self, destination_address, payer_kp=None):
        ...
        print(f"🧹 Sweeping all Solana assets to {destination_address}...")
        try: dest_pubkey = hex_to_pubkey(destination_address)
        except:
        ...
        with open("current_solana_wallets.json", "r") as f: wallets = json.load(f)
        service = Client(self.sol_url)

        if payer_kp:
            payer_bal = service.get_balance(payer_kp.pubkey()).value
            if payer_bal < 5000000:
                print(f"❌ Payer {payer_kp.pubkey()} has insufficient SOL. Sweep aborted.")
                return
                    ...
                    if token_accounts_res.value:
                        for acc in token_accounts_res.value:
                            ...
                            if amount > 0:
                                ...
                                instructions = []
                                if service.get_account_info(dest_ata).value is None:
                                    payer_p = payer_kp.pubkey() if payer_kp else kp.pubkey()
                                    instructions.append(create_associated_token_account(payer_p, dest_pubkey, mint))
                                instructions.append(transfer_checked(TransferCheckedParams(TOKEN_PROGRAM_ID, acc_pubkey, mint, dest_ata, kp.pubkey(), amount, bal_res.value.decimals)))
                                tx_p = {"instructions": instructions}
                                if payer_kp: tx_p["signers"], tx_p["fee_payer_kp"] = [payer_kp, kp], payer_kp
                                else: tx_p["sender_kp"] = kp
                                sig, err = sign_and_send_transaction(tx_p, service)
                                if sig: print(f"    ✅ Token sweep success! {sig[:12]}...")
                                else: print(f"    ❌ Token sweep fail: {err}")
                except Exception as e: print(f"    ⚠️ Token sweep failed for {addr_str[:8]}: {e}")

                try:
                    from solana.rpc.commitment import Confirmed
                    balance = service.get_balance(kp.pubkey(), commitment=Confirmed).value
                    buffer, fee_estimate = 1000000, 5000
                    if balance > (fee_estimate + buffer):
                        sweep_amount = balance - fee_estimate - buffer
                        print(f"    * Sweeping {sweep_amount/10**9} SOL...")
                        tx_d = build_transfer_tx(kp, [(str(dest_pubkey), sweep_amount)])
                        if payer_kp: tx_d["signers"], tx_d["fee_payer_kp"] = [payer_kp, kp], payer_kp
                        sig, err = sign_and_send_transaction(tx_d, service)
                        if sig: print(f"    ✅ SOL sweep success! {sig[:12]}...")
                        else: print(f"    ❌ SOL sweep fail: {err}")
                except Exception as e: print(f"    ⚠️ SOL sweep failed for {addr_str[:8]}: {e}")

    # --- EVM Specific Logic ---

    def check_all_evm_balances(self, case_file, evm_wallet_info):
        _, main_wallet, batch_wallets = evm_wallet_info
        cases = GetTestCase(case_file).get_test_cases()

        # Determine RPC from first case if possible
        rpc_url = cases[0].get('node_url', self.evm_url) if cases else self.evm_url
        w3 = Web3(Web3.HTTPProvider(rpc_url))

        print("\n" + "="*80)
        print(f"🔍 EVM Balance Diagnostics (via RPC: {rpc_url})")
        print("="*80)
        ...
        print("-" * 80)
        print(f"{'BATCH':<10} | {'EVM ADDRESS':<15} | {'ETH':<10} | {'TOKEN STATUS'}")
        print("-" * 80)
        for i, wallet in enumerate(batch_wallets):
            if i >= len(cases): break
            case = cases[i]
            ...
            token_status = "N/A"
            if case['from_token_address'].startswith('0x'):
                try:
                    # Simple ERC20 balance check
                    abi = [{"constant": True, "inputs": [{"name": "_owner", "type": "address"}], "name": "balanceOf", "outputs": [{"name": "balance", "type": "uint256"}], "type": "function"}]
                    contract = w3.eth.contract(address=Web3.to_checksum_address(case['from_token_address']), abi=abi)
                    t_bal = contract.functions.balanceOf(addr).call()
                    t_bal_f = t_bal / (10**case['from_token_decimals'])
                    t_needed_f = case['amount_raw'] / (10**case['from_token_decimals'])
                    t_check = "✅" if t_bal >= case['amount_raw'] else "❌"
                    token_status = f"{t_check} {case['token_name']}: {self.format_float(t_bal_f)}/{self.format_float(t_needed_f)}"
                except: token_status = "Error"

            print(f"{i+1:<10} | {addr[:12]}... | {w3.from_wei(balance, 'ether'):.4f} | {token_status}")
        print("="*80 + "\n")

    def distribute_evm_funds(self, case_file, evm_wallet_info):
        _, main_wallet, batch_wallets = evm_wallet_info
        cases = GetTestCase(case_file).get_test_cases()
        rpc_url = cases[0].get('node_url', self.evm_url) if cases else self.evm_url
        w3 = Web3(Web3.HTTPProvider(rpc_url))

        main_account = Account.from_key(main_wallet['private_key'])
        print(f"💸 Distributing EVM Funds from {main_account.address}...")

        nonce = w3.eth.get_transaction_count(main_account.address)
        for i, wallet in enumerate(batch_wallets):
            if i >= len(cases): break
            case = cases[i]
            dest_addr = Web3.to_checksum_address(wallet['address'])

            # Send ETH (0.01 ETH for gas)
            try:
                tx = {
                    'nonce': nonce,
                    'to': dest_addr,
            ...
            if case['from_token_address'].startswith('0x'):
                try:
                    ...
                    contract = w3.eth.contract(address=Web3.to_checksum_address(case['from_token_address']), abi=abi)
                    tx = contract.functions.transfer(dest_addr, int(case['amount_raw'])).build_transaction({
    ...
    def sweep_evm_assets(self, destination_address, evm_wallet_info, case_file=None):
        _, main_wallet, batch_wallets = evm_wallet_info
